...
### SingleObjectToXmlJackson 
##### _a Camel example project_

Functionality: **_Uses JacksonXML to create xml from a single object_**
...
git clone https://github.com/jammazwan/xao_SingleObjectToXmlJackson.git
cd jammazwan.shared
mvn install 
```
...
_run XaoTest to examine files generated_
...
so you can focus only on what is required to demo the **SingleObjectToXmlJackson** feature **_in isolation_**.
...
The prefix **xao** is used for maintaining sort order in a directory, providing packaging, and is otherwise meaningless. The x stands for _example_.
